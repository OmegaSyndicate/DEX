...
module.exports = async function (done) {
   ...
   try {
     ...
     console.log("Initiating DFP2 airdrop: ", dfpGov.address);
     console.log("Setting allowance for airdrop contract");
     await dfpGov.approve(airdrop.address, constants.MAX_UINT256);
     console.log("Airdropping DFP2");
     var amounts = [];
     for (const v of Object.values(DFP2)) {
       amounts.push(BigInt(v));
     }
     result = await airdrop.airdrop(dfpGov.address, Object.keys(DFP2), amounts);
     console.log(result);
